...
import ThumbnailImg from './components/ThumbnailImgItem'
...
class App extends Component {
  ...
  onClickThumbnailImg = id => {
    if (shuffledImagesList[0].id === id) {
      this.setState(prevState => ({score: prevState.score + 1}))

      //   const removeItem = imagesList.shift()
      //   imagesList.push(removeItem)

      //   //   console.log(imagesList)
      shuffledImagesList = [...imagesList].sort(() => Math.random() - 0.5)
    } else {
  ...
  renderWinnerCardView = () => {
    ...
    return (
      <div className="winner-card-container">
        ...
        <h1 className="digital-score-styles">{score}</h1>
        <button
          type="button"
          className="play-btn-styles"
          onClick={this.onClickPlayAgainButton}
        >
          <img
            src="https://assets.ccbp.in/frontend/react-js/match-game-play-again-img.png"
            alt="reset"
            className="reset-logo-styles"
          />
          PLAY AGAIN
        </button>
      </div>
  ...
  renderPlayingGameUi = () => {
    ...
    // const shuffledImagesList = filteredImagesList.sort(
    //   () => Math.random() - 0.5,
    // )
    ...
    return (
      <>
        <img
          src={shuffledImagesList[0].imageUrl}
          alt="match"
          className="main-img-styles"
        />
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetails={eachTab}
              isTabIdActive={eachTab.tabId === activeTab}
        ...
        <ul className="thumbnails-images-container">
          {filteredImagesList.map(eachImg => (
            <ThumbnailImg
              key={eachImg.id}
              eachThumbnailImgDetails={eachImg}
  ...
  render() {
    ...
    // const shuffledImagesList = imagesList.sort(() => Math.random() - 0.5)

    return (
      <div className="bg-container">
        <div className="app-container">
          <NavItem score={score} timer={timer} />
          <div className="game-ui-container">
            <div className="game-ui">
              {isGameOver || timer === 0
                ? this.renderWinnerCardView()
                : this.renderPlayingGameUi()}
            </div>
          </div>
        </div>
      </div>
